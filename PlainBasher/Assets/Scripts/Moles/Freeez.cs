...
public class Freeez : Mole
{
    ...
    void Start()
    {
        ...
        if (iceSteamPrefab)
        {
            Vector3 iceSteamPosition = transform.position;
            iceSteamPosition.y += iceSteamOffset;
            iceSteam = (GameObject)Instantiate(iceSteamPrefab, iceSteamPosition, transform.rotation);
            iceSteam.transform.parent = transform;
        }
    }
